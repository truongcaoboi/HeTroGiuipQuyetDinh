...
public class Execute {
	public Vector<Nongsan> taoDanhSach(String ma,Vector<Nongsan> v,double k,double t, int thang) throws SQLException{
		...
		if(con==null) {
			...
			while(rs.next()) {
				...
				double chiphi=rs.getDouble("chiphidukien");
				double nangsuat=rs.getDouble("nangsuattoida")*k*t;
	...
	public int check(Vector<Nongsan> v,int i,double dientich,double von) {
		double s=0;
		double chiphi=0;
		...
		Nongsan ns1=(Nongsan)v.get(i);
		double x=ns1.getDientich();
		if(x>70) {
			return -1;
		}else {
		if(s<100) {
...
/*		if(i<v.size()-1) {
	...
	public Vector<Phuongan> taoKetQua(Vector<Nongsan> ds,double dientich,double von) throws ArrayIndexOutOfBoundsException{
	...
	public void tinhLoiNhuanChiPhi(Vector<Phuongan> ketqua,double dientich) {
...
//			System.out.println("phuongan: "+i+"co chi phi: "+pa.getChiphi()+"co loi nhuan: "+pa.getLoinhuan());
	...
	public double maxLoiNhuan(Vector<Phuongan> ketqua) {
	...
	public double minChiPhi(Vector<Phuongan> ketqua) {
	...
	//	System.out.println("có: "+ketqua1.size()+" kết quả 1");
		if(ketqua1.size()==1) {
			...
			for(int i=0;i<ketqua1.size();i++) {
				...
				if(pa.getChiphi() <= minChiPhi) {
					ketqua2.addElement(pa);
				}
			}
	//		System.out.println("có: "+ketqua2.size()+" kết quả 2");
			if(ketqua2.isEmpty()) {
